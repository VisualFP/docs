...
As the aim of this project is to find a visual representation for functional programming,
the use case descriptions are kept very brief and only state the intention behind the use case, but not the manner in which it is to be achieved.
...
For that the user needs possiblities to distinct between a recursive and a base case.
...
A user wants to open a previously saved source file, so they can continue working on their program.
