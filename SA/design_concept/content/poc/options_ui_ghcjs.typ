...
There are many other Haskell libraries for UI implementations, but
many rely on compiling Haskell to JavaScript. This cross-compilation is often
based on GHCJS. GHCJS implements a JS backend to GHC and has recently been
merged into the GHC repository @ghcjs-merged.

Using GHCJS with an accompanying UI library, which would optimally support
...
- At the time of writing, there are no pre-built binaries available, which means
  that the complete GHC compiler needs to be built manually.
