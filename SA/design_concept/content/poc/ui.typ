...
== Features
The UI for the PoC application includes two main components, as shown in @ui-empty-editor: A sidebar with pre-defined value blocks and the function editor.
...
The PoC allows to define a function, the "userDefinedFunction", which starts with a generic type hole.
...
As the PoC is just intended to test the concept, there is a reset button to return to the initial empty definition.
...
The rendering part creates an HTML representation of each block in the value definition block and annotates it with CSS classes according to its block type.
...
While the first intention was to build the PoC with a FRP architecture, it became clear over time that Threepenny's FRP library is not yet ready for more complex use-cases like VisualFP's function editor.
The main problem is, that no function allows it to merge multiple events.
Implementing the FRP architecture through Threepenny could be considered again once the FRP library is replaced by reactive-banana #footnote("https://github.com/HeinrichApfelmus/reactive-banana").
